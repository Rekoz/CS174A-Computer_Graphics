...
function bowl( points_transform )
	{
		shape.call(this);
		this.populate(this, mat4() );
		this.init_buffers();
...
inherit(bowl, shape);
	...
	bowl.prototype.populate = function( recipient, points_transform )
		{
			var lastPoint = vec3(0.5, -0.5, 0);
			for ( var i = 0; i < 10; i++ )
			{
				var rotation = rotate((i+1)*36, 0, 0, 1);
				var newPoint4 = mult_vec( rotation, vec4(0.5, -0.5, 0, 1) );
				var newPoint = vec3(newPoint4[0], newPoint4[1], newPoint4[2]);
				recipient.vertices.push( vec3(0,0,0) );
				recipient.vertices.push( lastPoint );
				recipient.vertices.push( newPoint );
				recipient.indices.push(9*i);
				recipient.indices.push(9*i+1);
				recipient.indices.push(9*i+2);
				recipient.vertices.push( lastPoint );
				recipient.vertices.push( newPoint );
				var thirdPoint = vec3(lastPoint[0]*1.5, lastPoint[1]*1.5, lastPoint[2]*1.5+1);
				recipient.vertices.push( thirdPoint );
				recipient.indices.push(9*i+3);
				recipient.indices.push(9*i+4);
				recipient.indices.push(9*i+5);
				recipient.vertices.push( thirdPoint );
				recipient.vertices.push( newPoint );
				thirdPoint = vec3(newPoint[0]*1.5, newPoint[1]*1.5, newPoint[2]*1.5+1);
				recipient.vertices.push( thirdPoint );
				recipient.indices.push(9*i+6);
				recipient.indices.push(9*i+7);
				recipient.indices.push(9*i+8);
				lastPoint = newPoint;
			}
			recipient.flat_normals_from_triples(0);
		}

function ufo( points_transform )
	{
		shape.call(this);
		this.populate(this, mat4() );
		this.init_buffers();
...
inherit(ufo, shape);
	...
	ufo.prototype.populate = function( recipient, points_transform )
		{
			var lastPoint = vec3(0.5, -0.5, 0);
			recipient.vertices.push( vec3(0,0,0) );
			...
			recipient.vertices.push( lastPoint );
			recipient.normals.push( subtract(lastPoint, vec3(0,0,1)) );
			recipient.indices.push(1);
			var thirdPoint = vec3(lastPoint[0]*1.5, lastPoint[1]*1.5, lastPoint[2]*1.5+1);
			recipient.vertices.push(thirdPoint);
			recipient.normals.push( subtract(thirdPoint, vec3(0,0,1)) );
			recipient.indices.push(2);
			for ( var i = 1; i < 20; i++ )
			{
				var rotation = rotate((i+1)*18, 0, 0, 1);
				var newPoint4 = mult_vec( rotation, vec4(0.5, -0.5, 0, 1) );
				var newPoint = vec3(newPoint4[0], newPoint4[1], newPoint4[2]);
				recipient.vertices.push( newPoint );
				recipient.normals.push( subtract(newPoint, vec3(0,0,1)) );
				recipient.indices.push(0);
				recipient.indices.push(2*(i-1)+1);
				recipient.indices.push(2*i+1);
				recipient.indices.push(2*(i-1)+1);
				recipient.indices.push(2*i+1);
				recipient.indices.push(2*(i-1)+2);
				thirdPoint = vec3(newPoint[0]*1.5, newPoint[1]*1.5, newPoint[2]*1.5+1);
				recipient.vertices.push( thirdPoint );
				recipient.normals.push( subtract(thirdPoint, vec3(0,0,1)) );
				recipient.indices.push(2*i+1);
